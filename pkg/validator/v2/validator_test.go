...
func TestCheckChangeConstraintsBucketMigratingAnnotation(t *testing.T) {
	magma := couchbasev2.CouchbaseStorageBackendMagma

	testcases := []struct {
		name                   string
		prevRoutines           bool
		migrationConditionTrue bool
		currentRoutines        bool
		currentTargetBackend   *couchbasev2.CouchbaseStorageBackend
		expectedErrContains    string
	}{
		{
			name:                   "block routines toggle while migration in progress",
			prevRoutines:           true,
			migrationConditionTrue: true,
			currentRoutines:        false,
			currentTargetBackend:   &magma,
			expectedErrContains:    "cannot be changed while a bucket migration is taking place",
		},
		{
			name:                 "block disabling routines while annotation still set",
			prevRoutines:         true,
			currentRoutines:      false,
			currentTargetBackend: &magma,
			expectedErrContains:  "cannot be set to false while the cao.couchbase.com/buckets.targetUnmanagedBucketStorageBackend annotation is set",
		},
		{
			name:                 "allow disabling routines when annotation cleared in same update",
			prevRoutines:         true,
			currentRoutines:      false,
			currentTargetBackend: nil,
			expectedErrContains:  "",
		},
		{
			name:                 "allow enabling routines (false to true)",
			prevRoutines:         false,
			currentRoutines:      true,
			currentTargetBackend: &magma,
			expectedErrContains:  "",
		},
		{
			name:                 "allow no-op (routines unchanged)",
			prevRoutines:         true,
			currentRoutines:      true,
			currentTargetBackend: &magma,
			expectedErrContains:  "",
	...
	for _, tc := range testcases {
		prev := &couchbasev2.CouchbaseCluster{
			Spec: couchbasev2.ClusterSpec{
				Buckets: couchbasev2.Buckets{EnableBucketMigrationRoutines: tc.prevRoutines},
			},
		}
		if tc.migrationConditionTrue {
			prev.Status = couchbasev2.ClusterStatus{
				Conditions: []couchbasev2.ClusterCondition{{
					Type:   couchbasev2.ClusterConditionBucketMigration,
					Status: v1.ConditionTrue,
				}},
			}
		}

		current := &couchbasev2.CouchbaseCluster{
			Spec: couchbasev2.ClusterSpec{
				Buckets: couchbasev2.Buckets{
					EnableBucketMigrationRoutines:       tc.currentRoutines,
					TargetUnmanagedBucketStorageBackend: tc.currentTargetBackend,
				},
			},
		}

		err := checkChangeConstraintsBucketMigratingAnnotation(prev, current)
		...
		if err == nil {
			t.Errorf("%s: expected error containing %q, got nil", tc.name, tc.expectedErrContains)
			continue
		}
		if !strings.Contains(err.Error(), tc.expectedErrContains) {
			t.Errorf("%s: expected error containing %q, got %v", tc.name, tc.expectedErrContains, err)
		}
	}
}

func TestCheckConstraintBucketsAnnotationsTargetBackendRequiresRoutines(t *testing.T) {
	const annotationKey = "cao.couchbase.com/buckets.targetUnmanagedBucketStorageBackend"

	testcases := []struct {
		name                string
		annotationValue     string // "" means don't set the annotation at all
		enableRoutines      bool
		expectedErrContains string
	}{
		{
			name:                "reject annotation when routines disabled",
			annotationValue:     "magma",
			enableRoutines:      false,
			expectedErrContains: "requires bucket migration routines to be enabled",
		},
		{
			name:                "allow annotation when routines enabled",
			annotationValue:     "magma",
			enableRoutines:      true,
			expectedErrContains: "",
		},
		{
			name:                "allow absence of annotation regardless of routines flag",
			annotationValue:     "",
			enableRoutines:      false,
			expectedErrContains: "",
		},
	...
	for _, tc := range testcases {
		cluster := &couchbasev2.CouchbaseCluster{
			...
			Spec: couchbasev2.ClusterSpec{
				Image:   "couchbase/server:7.6.2",
				Buckets: couchbasev2.Buckets{EnableBucketMigrationRoutines: tc.enableRoutines},
			},
		}
		if tc.annotationValue != "" {
			cluster.Annotations = map[string]string{annotationKey: tc.annotationValue}
		}

		err := checkConstraintBucketsAnnotations(nil, cluster)
