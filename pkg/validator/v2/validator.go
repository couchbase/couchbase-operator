...
func CheckConstraintsEncryptionKey(v *types.Validator, key *couchbasev2.CouchbaseEncryptionKey) error {
	...
	// Check that rotation start time is in the future if specified'
	if key.Spec.AutoGenerated != nil && key.Spec.AutoGenerated.Rotation != nil && key.Spec.AutoGenerated.Rotation.StartTime != nil {
		startTime := key.Spec.AutoGenerated.Rotation.StartTime.Time
		if startTime.Before(metav1.Now().Time) {
			errs = append(errs, fmt.Errorf("spec.autoGenerated.rotation.startTime must be in the future"))
		}
	}
...
func checkConstraintEncryptionKeys(v *types.Validator, cluster *couchbasev2.CouchbaseCluster) error {
	if cluster.Spec.Security.EncryptionAtRest == nil || !cluster.Spec.Security.EncryptionAtRest.Enabled {
		return nil
	}
