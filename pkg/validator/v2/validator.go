...
func checkBucketEncryptionAtRestSettings(v *types.Validator, bucket *couchbasev2.CouchbaseBucket) error {
	...
	for _, cluster := range clusters {
		...
		if !key.CanEncryptBucket(bucket.GetCouchbaseName()) {
			return fmt.Errorf("spec.encryptionAtRest.keyName %s does not have bucket encryption usage for bucket %s on cluster %s", keyName, bucket.GetCouchbaseName(), cluster.Name)
		}
	}
	return nil
}
...
func CheckConstraintsEncryptionKey(v *types.Validator, key *couchbasev2.CouchbaseEncryptionKey) ([]string, error) {
	...
	// Check that at least one of the usage values is set
	if !usage.Configuration && !usage.Key && !usage.Log && !usage.Audit && !usage.AllBuckets && len(usage.Buckets) == 0 {
		errs = append(errs, fmt.Errorf("at least one usage field must be set in spec.usage"))
	}

	// The wildcard supersedes the list, so it has no effect. Warn rather than
	// reject: the key still works, it just permits more than the list asked for.
	if usage.AllBuckets && len(usage.Buckets) > 0 {
		warnings = append(warnings, "spec.usage.buckets is ignored because spec.usage.allBuckets is true: set allBuckets to false to restrict the key to the listed buckets")
	}
...
// checkUsageBuckets checks spec.usage.buckets against one cluster: the release has
// to report usedBy, and every name has to be a bucket the cluster provides. The
// server rejects the whole key if any one name is unknown, taking out every other
// usage on it. Names are Couchbase names, which spec.name may override.
func checkUsageBuckets(v *types.Validator, key *couchbasev2.CouchbaseEncryptionKey, cluster *couchbasev2.CouchbaseCluster) []error {
	usage := key.GetUsage()
	...
	// Without usedBy a removal is rejected with "usage in use" and fails the
	// reconcile, so gate the field rather than allow an uneditable configuration.
	supportsUsedBy, err := cluster.IsAtLeastVersion(constants.MinimumVersionForEncryptionKeyUsedBy)
	if err != nil {
		return []error{err}
	}

	if !supportsUsedBy {
		return []error{fmt.Errorf("spec.usage.buckets can only be set for Couchbase Server %s+, cluster %s is older: use spec.usage.allBuckets instead",
			constants.MinimumVersionForEncryptionKeyUsedBy, cluster.Name)}
	}

	// Unmanaged buckets have no resources to check against.
	if !cluster.Spec.Buckets.Managed {
		return nil
	}
	...
	couchbaseNames := map[string]bool{}
	byResourceName := map[string]string{}

	for _, bucket := range buckets {
		couchbaseNames[bucket.GetCouchbaseName()] = true
		byResourceName[bucket.GetResourceName()] = bucket.GetCouchbaseName()
	}

	errs := []error{}

	for _, name := range usage.Buckets {
		if couchbaseNames[name] {
			continue
		}

		if couchbaseName, ok := byResourceName[name]; ok {
			errs = append(errs, fmt.Errorf("spec.usage.buckets entry %s is a resource name: use bucket name %s instead", name, couchbaseName))
			continue
		}

		errs = append(errs, fmt.Errorf("spec.usage.buckets references bucket %s which does not exist on cluster %s", name, cluster.Name))
	}
...
func checkEncryptionKeyUsageForCluster(v *types.Validator, key *couchbasev2.CouchbaseEncryptionKey, cluster *couchbasev2.CouchbaseCluster) []error {
	...
	errs = append(errs, checkUsageBuckets(v, key, cluster)...)
...
func checkEncryptionKeyUsageForClusterBuckets(v *types.Validator, key *couchbasev2.CouchbaseEncryptionKey, cluster *couchbasev2.CouchbaseCluster) []error {
	...
	for _, bucket := range couchbaseBuckets.Items {
		...
		// usage.buckets holds Couchbase names, which spec.name may override.
		if !key.CanEncryptBucket(bucket.GetCouchbaseName()) {
			errs = append(errs, fmt.Errorf("encryption key %s is used for encryption at rest on bucket %s but neither spec.usage.allBuckets nor spec.usage.buckets permits it", key.Name, bucket.GetCouchbaseName()))
		}
	}
...
func validateEncryptionKeysUsedOnBuckets(v *types.Validator, cluster *couchbasev2.CouchbaseCluster, keyMap map[string]*couchbasev2.CouchbaseEncryptionKey) ([]string, error) {
	...
	for _, bucket := range couchbaseBuckets.Items {
		...
		if !key.CanEncryptBucket(bucket.GetCouchbaseName()) {
			errs = append(errs, fmt.Errorf("encryption key %s does not have bucket usage enabled for bucket %s", bucket.Spec.EncryptionAtRest.KeyName, bucket.GetCouchbaseName()))
			continue
		}
	}
