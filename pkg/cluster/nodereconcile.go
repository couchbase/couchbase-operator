...
type ReconcileMachine struct {
	...
	// ConstrainedRebalanceOut rollback whose pods live until handleEjectedMembers.
...
func (r *ReconcileMachine) handleAddNode(c *Cluster) error {
	...
	// live until handleEjectedMembers, so replacements wait until next cycle.
...
func (c *Cluster) rollbackMethod() couchbasev2.RollbackMethod {
	...
	case couchbasev2.RollbackMethodConstrainedFailover, couchbasev2.RollbackMethodConstrainedRebalanceOut:
		return method
	default:
		c.log.Info("[WARN] Unrecognised rollback method, using SwapRebalance",
...
func (r *ReconcileMachine) rollbackBatchSize(c *Cluster, method couchbasev2.RollbackMethod, candidates int) (int, error) {
	...
	if batch == 0 {
		c.log.Info("[WARN] No safe rollback batch for this topology, reverting to SwapRebalance",
	...
	if batch < candidates {
		c.log.Info("Reducing rollback batch to stay within safety limits",
			"cluster", c.namespacedName(), "rollbackMethod", method, "candidates", candidates,
...
// handleEjectedMembers, additions are held, and replacements land next cycle, costing
...
func (r *ReconcileMachine) tryRollbackWithoutSpareCapacity(c *Cluster, candidates couchbaseutil.MemberSet) (bool, error) {
	...
	if !r.couchbase.FailedNodes.Empty() || !r.couchbase.AddBackNodes.Empty() {
		c.log.Info("Deferring rollback while nodes are recovering", "cluster", c.namespacedName(),
	...
	c.log.Info("Rolling back pods without spare capacity", "cluster", c.namespacedName(),
	...
	for _, candidate := range candidates {
		...
		if method == couchbasev2.RollbackMethodConstrainedFailover {
			...
			// replaces. handleEjectedMembers then destroys it.
			if err := k8sutil.SetPodPendingUpgradeBeforeEjection(c.k8s, member.Name()); err != nil {
				return false, err
			}
