...
func (r *ReconcileMachine) handleInit(c *Cluster) {
	...
	if !r.couchbase.PendingAddNodes.Empty() || !r.couchbase.FailedNodes.Empty() ||
		!r.couchbase.AddBackNodes.Empty() || !r.couchbase.DownNodes.Empty() ||
		!r.couchbase.FailedAddNodes.Empty() || len(r.couchbase.UnmanagedNodes) > 0 ||
		!r.runningPods.Equal(c.members) || !r.couchbase.UnclusteredNodes.Empty() ||
	...
	if !r.couchbase.AddBackNodes.Empty() {
		c.logger.Infof("add back nodes: %s", r.couchbase.AddBackNodes)
	}
