...
func (c *Cluster) getBucketMigrationCandidates() (candidates couchbaseutil.MemberSet, cyclesNeeded bool, hasStorageBackendOverrides bool, err error) {
	...
	return candidates, cyclesNeeded, hasStorageBackendOverrides, nil
}
...
func (r *ReconcileMachine) handleBucketStorageBackendMigration(c *Cluster) error {
	...
	candidates, cyclesNeeded, hasStorageBackendOverrides, err := c.getBucketMigrationCandidates()
	if err != nil {
		return err
	}
...
func (r *ReconcileMachine) failoverMigrateMembers(c *Cluster, members couchbaseutil.MemberSet, fullRecovery bool) error {
	...
	for _, candidate := range members {
		log.Info("Migrating pod with failover and recovery", "cluster", c.namespacedName(),
