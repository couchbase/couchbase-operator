...
func (c *Cluster) reconcileAdminPassword() error {
	...
	if !k8sutil.PasswordCompliesWithCouchbasePasswordPolicy(c.cluster.Spec.Security.PasswordPolicy, password) {
		c.log.Info("Unable to rotate admin password as it does not comply with the current password policy", "cluster", c.namespacedName())
		return nil
	}

	c.log.Info("Rotating admin password", "cluster", c.namespacedName())
