...
func TestUnmarshalEncryptionKeys(t *testing.T) {
	managedKeyJSONString := `{
        ...
        "usedBy": [
            {"usage": "bucket-encryption-beer", "description": "bucket \"beer\""},
            {"usage": "config-encryption", "description": "configuration"}
        ],
        "creationDateTime": "2025-08-21T10:39:32Z"
    }`
	...
	// usage carries the wildcard, usedBy always names buckets individually.
	expectedUsedBy := []EncryptionKeyUsedBy{
		{Usage: "bucket-encryption-beer", Description: `bucket "beer"`},
		{Usage: "config-encryption", Description: "configuration"},
	}

	if !reflect.DeepEqual(managedKey.UsedBy, expectedUsedBy) {
		t.Errorf("Expected usedBy to be %v but got %v", expectedUsedBy, managedKey.UsedBy)
	}

	// usedBy is read-only, so it must never be sent back on create or update.
	encoded, err := json.Marshal(managedKey.EncryptionKey)
	if err != nil {
		t.Errorf("Error marshalling managed key: %v", err)
	}

	if strings.Contains(string(encoded), "usedBy") {
		t.Errorf("Expected marshalled key not to contain usedBy but got %s", string(encoded))
	}
...
// TestCanEncryptBucket checks that a key's usage is read as a permit for one named
// bucket, with the wildcard standing in for every bucket.
func TestCanEncryptBucket(t *testing.T) {
	cases := []struct {
		name   string
		usage  []string
		bucket string
		...
		{name: "no usage", usage: nil, bucket: "beer", want: false},
		{name: "wildcard", usage: []string{"bucket-encryption"}, bucket: "beer", want: true},
		{name: "exact match", usage: []string{"bucket-encryption-beer"}, bucket: "beer", want: true},
		{name: "different bucket", usage: []string{"bucket-encryption-travel"}, bucket: "beer", want: false},
		{name: "prefix is not a match", usage: []string{"bucket-encryption-beersample"}, bucket: "beer", want: false},
		{name: "unrelated usage", usage: []string{"config-encryption"}, bucket: "beer", want: false},
	}

	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			key := &EncryptionKeyInfo{EncryptionKey: EncryptionKey{Usage: tc.usage}}
			if got := key.CanEncryptBucket(tc.bucket); got != tc.want {
				t.Errorf("CanEncryptBucket(%q) = %v, want %v", tc.bucket, got, tc.want)
			}
		})
	}
}

// TestUsagesToRetain checks which usages have to be kept on an update because the
// server still reports DEKs for them, and would reject the whole update otherwise.
func TestUsagesToRetain(t *testing.T) {
	usedBy := func(usages ...string) []EncryptionKeyUsedBy {
		out := make([]EncryptionKeyUsedBy, 0, len(usages))
		for _, u := range usages {
			out = append(out, EncryptionKeyUsedBy{Usage: u})
		}
	...
	cases := []struct {
	...
	}{
		{
			name:      "nothing in use",
			usedBy:    nil,
			requested: []string{"config-encryption"},
			want:      nil,
		},
		{
			name:      "everything already requested",
			usedBy:    usedBy("config-encryption", "bucket-encryption-beer"),
			requested: []string{"config-encryption", "bucket-encryption-beer"},
			want:      nil,
		},
		{
			name:      "bucket usage being dropped is retained",
			usedBy:    usedBy("bucket-encryption-beer"),
			requested: []string{"config-encryption"},
			want:      []string{"bucket-encryption-beer"},
		},
		{
			name:      "wildcard covers a bucket specific usage",
			usedBy:    usedBy("bucket-encryption-beer", "bucket-encryption-travel"),
			requested: []string{"bucket-encryption"},
			want:      nil,
		},
		{
			name:      "a bucket specific usage does not cover the wildcard",
			usedBy:    usedBy("bucket-encryption"),
			requested: []string{"bucket-encryption-beer"},
			want:      []string{"bucket-encryption"},
		},
		{
			name:      "duplicate KEK entries are collapsed",
			usedBy:    usedBy("KEK-encryption", "KEK-encryption"),
			requested: nil,
			want:      []string{"KEK-encryption"},
		},
		{
			name:      "config, log and audit are retained too",
			usedBy:    usedBy("config-encryption", "log-encryption", "audit-encryption"),
			requested: []string{"bucket-encryption"},
			want:      []string{"config-encryption", "log-encryption", "audit-encryption"},
		},
	}

	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			key := &EncryptionKeyInfo{UsedBy: tc.usedBy}
			if got := key.UsagesToRetain(tc.requested); !reflect.DeepEqual(got, tc.want) {
				t.Errorf("UsagesToRetain() = %v, want %v", got, tc.want)
			}
		})
	}
}
