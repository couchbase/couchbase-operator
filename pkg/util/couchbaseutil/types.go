...
const (
	// EncryptionKeyTypeCBServerManagedAES256 is a Couchbase server managed AES-256 key.
	EncryptionKeyTypeCBServerManagedAES256 EncryptionKeyType = "cb-server-managed-aes-key-256"
	// EncryptionKeyTypeAWSKMSSymmetric is an AWS KMS symmetric key.
	EncryptionKeyTypeAWSKMSSymmetric EncryptionKeyType = "awskms-symmetric-key"
	// EncryptionKeyTypeKMIPAES256 is a KMIP-managed AES-256 key.
	EncryptionKeyTypeKMIPAES256 EncryptionKeyType = "kmip-aes-key-256"
)

// Common encryption key usage patterns.
const (
	EncryptionKeyUsageBucketEncryptionAll = "bucket-encryption"
	EncryptionKeyUsageConfigEncryption    = "config-encryption"
	EncryptionKeyUsageKEKEncryption       = "KEK-encryption"
	EncryptionKeyUsageLogsEncryption      = "log-encryption"
	EncryptionKeyUsageAuditEncryption     = "audit-encryption"
)

const (
	EncryptionKeyEncryptWithNodeSecretManager = "nodeSecretManager"
	EncryptionKeyEncryptWithEncryptionKey     = "encryptionKey"
)

// AutoGeneratedKeyData contains settings for auto-generated encryption keys.
type AutoGeneratedKeyData struct {
	AutoRotation           bool   `json:"autoRotation"`
	RotationIntervalInDays int    `json:"rotationIntervalInDays,omitempty"`
	NextRotationTime       string `json:"nextRotationTime,omitempty"` // ISO8601 datetime format
	CanBeCached            bool   `json:"canBeCached"`
	EncryptWith            string `json:"encryptWith"`
	EncryptWithKeyID       *int   `json:"encryptWithKeyId,omitempty"`
}

// EncryptionKey is what we send to create/update the encryption key.
type EncryptionKey struct {
	Type  EncryptionKeyType `json:"type"`
	Name  string            `json:"name"`
	Usage []string          `json:"usage"`

	// To make the API easier to work with we have these fields that are
	// used to store the data for the different key types.
	AutoGenerated *AutoGeneratedKeyData `json:"-"`
	AWS           *AWSKeyData           `json:"-"`
	KMIP          *KMIPKeyData          `json:"-"`
}

type EncryptionKeyList []EncryptionKeyInfo

func (l EncryptionKeyList) GetKeyByName(name string) *EncryptionKeyInfo {
	for _, key := range l {
		if key.Name == name {
			return &key
		}
...
func (e EncryptionKey) MarshalJSON() ([]byte, error) {
	type _EncryptionKey EncryptionKey

	key := struct {
		_EncryptionKey
		Data interface{} `json:"data"`
	}{
		_EncryptionKey: _EncryptionKey(e),
	}

	switch e.Type {
	case EncryptionKeyTypeCBServerManagedAES256:
		key.Data = e.AutoGenerated
	case EncryptionKeyTypeAWSKMSSymmetric:
		key.Data = e.AWS
	case EncryptionKeyTypeKMIPAES256:
		key.Data = e.KMIP
	}
...
// UnmarshalJSON is used to unmarshal the EncryptionKey from a JSON object.
func (e *EncryptionKeyInfo) UnmarshalJSON(data []byte) error {
	type _EncryptionKey EncryptionKeyInfo
	...
	*e = (EncryptionKeyInfo)(temp._EncryptionKey)

	switch e.Type {
	case EncryptionKeyTypeCBServerManagedAES256:
		err = json.Unmarshal(temp.Data, &e.AutoGenerated)
	case EncryptionKeyTypeAWSKMSSymmetric:
		err = json.Unmarshal(temp.Data, &e.AWS)
	case EncryptionKeyTypeKMIPAES256:
		err = json.Unmarshal(temp.Data, &e.KMIP)
	}
...
// AWSKeyData contains settings for AWS KMS encryption keys.
type AWSKeyData struct {
	KeyARN          string `json:"keyARN"`
	Region          string `json:"region"`
	UseIMDS         bool   `json:"useIMDS"`
...
type AllEncryptionAtRestSettings struct {
	Config EncryptionAtRestSettings `json:"config,omitempty"`
