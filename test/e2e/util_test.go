...
func registerTests() {
	framework.TestDefinitions = framework.TestDefList{
		...
		framework.NewTestDef(TestRzaNegCreateCluster).WithTags(TagSuiteSanity, TagFeatureServerGroups),
		...
		framework.NewTestDef(TestTaintK8SNodeAndRemoveTaint).WithTags(TagSuiteP1, TagFeatureRecovery),
