...
func TestRecoveryAfterOneNodeUnreachableBucketOneReplica(t *testing.T) {
	...
	e2eutil.MustExecShellInPod(t, targetKube, targetKube.Namespace, memberName, "iptables -A INPUT -p tcp -s 0/0 -d $(/bin/hostname -i) --sport 513:65535 --dport 22 -m state --state NEW,ESTABLISHED -j ACCEPT; iptables -A OUTPUT -p tcp -s $(/bin/hostname -i) -d 0/0 --sport 22 --dport 513:65535 -m state --state ESTABLISHED -j ACCEPT")
...
func TestRecoveryNodeTmpUnreachableBucketOneReplica(t *testing.T) {
	...
	e2eutil.MustExecShellInPod(t, targetKube, targetKube.Namespace, memberName, "iptables -A INPUT -p tcp -s 0/0 -d $(/bin/hostname -i) --sport 513:65535 --dport 22 -m state --state NEW,ESTABLISHED -j ACCEPT")
	e2eutil.MustExecShellInPod(t, targetKube, targetKube.Namespace, memberName, "iptables -A OUTPUT -p tcp -s $(/bin/hostname -i) -d 0/0 --sport 22 --dport 513:65535 -m state --state ESTABLISHED -j ACCEPT")
	time.Sleep(autofailoverTimeout / 2)
	e2eutil.MustExecShellInPod(t, targetKube, targetKube.Namespace, memberName, "iptables -F")
	e2eutil.MustExecShellInPod(t, targetKube, targetKube.Namespace, memberName, "iptables -X")
	e2eutil.MustExecShellInPod(t, targetKube, targetKube.Namespace, memberName, "iptables -P INPUT DROP")
	e2eutil.MustExecShellInPod(t, targetKube, targetKube.Namespace, memberName, "iptables -P OUTPUT DROP")
	e2eutil.MustExecShellInPod(t, targetKube, targetKube.Namespace, memberName, "iptables -P FORWARD DROP")
	e2eutil.MustWaitUntilPodSizeReached(t, targetKube, testCouchbase, 4, 5*time.Minute)
	e2eutil.MustWaitUntilPodSizeReached(t, targetKube, testCouchbase, 5, 5*time.Minute)
	e2eutil.MustWaitClusterStatusHealthy(t, targetKube, testCouchbase, 2*time.Minute)
