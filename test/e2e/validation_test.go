...
func TestValidationEncryptionKey(t *testing.T) {
	testDefs := []testDef{
		{
			name:           "TestEncryptionKeyRotationStartTime",
			mutations:      patchMap{"auto-generated-key-1": jsonpatch.NewPatchSet().Replace("/spec/autoGenerated/rotation/startTime", "1996-11-11T11:11:11Z")},
			shouldFail:     true,
			expectedErrors: []string{"spec.autoGenerated.rotation.startTime must be in the future"},
		},
		{
...
func TestValidationEncryptionAtRest(t *testing.T) {
	testDefs := []testDef{
		...
		{
			name: "TestEncryptionAtRestDisabledConfiguration",
			mutations: patchMap{"cluster1": jsonpatch.NewPatchSet().Add("/spec/security/encryptionAtRest", &couchbasev2.EncryptionAtRestSpec{
				Enabled: true,
				Configuration: &couchbasev2.EncryptionAtRestUsageConfiguration{
					Enabled: false,
			...
			shouldFail: false, // Disabled configuration should not validate the key
		},
		{
