...
exitOnError $? "Unable to create tar of testrunner platform-cert docker image"
...
while true
    do
        testrunnerPodName=$(kubectl --namespace=$KUBENAMESPACE get -l job-name=testrunner-platform-cert pods | tail -1 | awk '{print $1}')
...
while true
do
    currTestrunnerPod=$(kubectl --namespace=$KUBENAMESPACE get -l job-name=testrunner-platform-cert pods | tail -1 | awk '{print $1}')
