...
type K8SClustersGetterSetter interface {
	...
	DeleteAKSClusterDetail(clusterName string) error
	DeleteGKEClusterDetail(clusterName string) error
}
...
func (ks *K8SClusters) DeleteAKSClusterDetail(clusterName string) error {
	ks.mu.Lock()
	defer ks.mu.Unlock()

	if _, exists := ks.aksClusters[clusterName]; exists {
		delete(ks.aksClusters, clusterName)
	} else {
		return fmt.Errorf("delete aks cluster detail: %w", ErrClusterNotFound)
	}

	return nil
}

func (ks *K8SClusters) DeleteGKEClusterDetail(clusterName string) error {
	ks.mu.Lock()
	defer ks.mu.Unlock()

	if _, exists := ks.gkeClusters[clusterName]; exists {
		delete(ks.gkeClusters, clusterName)
	} else {
		return fmt.Errorf("delete gke cluster detail: %w", ErrClusterNotFound)
	}
			...
			case managedk8sservices.Kubernetes:
				...
				case managedk8sservices.Cloud:
					...
					case managedk8sservices.Azure:
						if err := ks.PopulateAKSCluster(cluster); err != nil {
							return fmt.Errorf("populate all clusters: %w", err)
						}
					case managedk8sservices.GCP:
						if err := ks.PopulateGKECluster(cluster); err != nil {
							return fmt.Errorf("populate all clusters: %w", err)
						}
					default:
...
func (ks *K8SClusters) PopulateAKSCluster(clusterName string) error {
	k8sCluster := &K8SCluster{
		clusterName:     clusterName,
		serviceProvider: managedk8sservices.NewManagedServiceProvider(managedk8sservices.Kubernetes, managedk8sservices.Cloud, managedk8sservices.Azure),
	}

	aksClusterDetail := &AKSClusterDetail{
	...
	if err := aksClusterDetail.PopulateAKSClusterDetail(); err != nil {
		return fmt.Errorf("populate aks cluster: %w", err)
	}
...
func (ks *K8SClusters) PopulateGKECluster(clusterName string) error {
	k8sCluster := &K8SCluster{
		clusterName:     clusterName,
		serviceProvider: managedk8sservices.NewManagedServiceProvider(managedk8sservices.Kubernetes, managedk8sservices.Cloud, managedk8sservices.GCP),
	}

	gkeClusterDetail := &GKEClusterDetail{
	...
	if err := gkeClusterDetail.PopulateGKEClusterDetail(); err != nil {
		return fmt.Errorf("populate gke cluster: %w", err)
	}
