...
Kubernetes is a cluster and orchestration engine for https://opencontainers.org/[OCI-compliant^] containers. In other words Kubernetes is an open source software or tool which is used to orchestrate and manage containers in a cluster environment. Kubernetes is also known as k8s and it was developed by Google and donated to "`Cloud Native Computing foundation`"

In Kubernetes setup we have one or more master nodes and multiple other nodes. Cluster nodes are known as worker nodes or minions. From the master nodes we manage the cluster and its nodes using '`kubeadm`' and '`kubectl`'  command.
...
In this guide we will install latest version of Kubernetes on CentOS 7 / RHEL 7 with kubeadm utility. In this setup we are taking three CentOS 7 servers with minimal installation. One server will act as a master node and the other two servers will be minion or worker nodes. The container runtime used will be docker.
...
* Kubelet -- It is an agent which runs on every worker node, it connects to the container runtime and takes care of creating, starting, deleting containers.
...
* Pod -- Pod can be defined as a multi-tier or group of containers that are deployed on a single worker node or container host.
