...
Since we'll primarily be working with gerrit, ensure your local ssh agent's config file has the following in addition to github.com host.
...
Inside there is an artifacts folder, containing the test `results.xml` and `logs` folder with logs for every test, plus a xref:tools/cao.adoc[`cao collect-logs`] archive for any failed test.
You can unzip this with `tar -xf <FILE>`, and view an individual instances logs via `./namespace/pod/<PODNAME>/`

https://github.com/spjmurray/logify/[Logify^] can be used to pull apart the logs collected via `cao collect-logs`.
