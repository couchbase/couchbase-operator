...
*Enumerations*: `slient, fatal, error, warn, info, verbose, timing, debug, trace`
...
IndexClaim s a persistent volume that encompasses index storage associated with the index service.
The index claim can only be used on server classes running the index service, and must be used in conjunction with the default claim.
This field allows the index service to use different storage media (e.g.
SSD) to improve performance of this service.
This field references a volume claim template name as defined in "spec.volumeClaimTemplates".
...
The logs claim can only be used on server classes running stateless services: query, search & eventing.
