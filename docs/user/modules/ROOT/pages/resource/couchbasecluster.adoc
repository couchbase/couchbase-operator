...
If not specified, or missing the requests.memory field, then the Operator will automatically populate memory requests by calculating the total memory requirements for each service enabled for this class, then scaling by an overhead.
