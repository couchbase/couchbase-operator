...
<<couchbaseclusters-spec,spec>>:
  ...
  <<couchbaseclusters-spec-cluster,cluster>>:
    ...
    <<couchbaseclusters-spec-cluster-query,query>>:
      ...
      <<couchbaseclusters-spec-cluster-query-nodequota,nodeQuota>>: ""
...
CouchbaseEphemeralBucket or CouchbaseMemcachedBucket resources.
Manual addition
...
=== couchbaseclusters.spec.cluster.query.nodeQuota
...
NodeQuota sets a soft memory limit for every Query node in the cluster.
The garbage
collector tries to keep below this target.
...
By default, Couchbase server provides no
memory resource constraints for the query service, so this has no effect on Couchbase
server.
It is, however, used when the spec.autoResourceAllocation feature is enabled,
...
When DeltaRecovery is requested, the operator will
perform an in-place upgrade on a best effort basis.
DeltaRecovery cannot be used if the UpgradeStrategy
