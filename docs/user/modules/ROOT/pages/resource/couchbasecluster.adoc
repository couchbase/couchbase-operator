...
<<couchbaseclusters-spec,spec>>:
  ...
  <<couchbaseclusters-spec-buckets,buckets>>:
    ...
    <<couchbaseclusters-spec-buckets-selector,selector>>:
      ...
      <<couchbaseclusters-spec-buckets-selector-matchnames,matchNames>>:
      - ""
  ...
  <<couchbaseclusters-spec-cluster,cluster>>:
    ...
    <<couchbaseclusters-spec-cluster-data,data>>:
      ...
      <<couchbaseclusters-spec-cluster-data-kvthrottle,kvThrottle>>:
        <<couchbaseclusters-spec-cluster-data-kvthrottle-enabled,enabled>>: false
        <<couchbaseclusters-spec-cluster-data-kvthrottle-nodecapacity,nodeCapacity>>: 0
      <<couchbaseclusters-spec-cluster-data-magmaflusherthreadpercentage,magmaFlusherThreadPercentage>>: 0
    ...
    <<couchbaseclusters-spec-cluster-indexstoragesetting,indexStorageSetting>>: ""
    ...
    <<couchbaseclusters-spec-cluster-rebalance,rebalance>>:
      <<couchbaseclusters-spec-cluster-rebalance-filebasedmovespernode,fileBasedMovesPerNode>>: 0
      <<couchbaseclusters-spec-cluster-rebalance-movespernode,movesPerNode>>: 0
  ...
  <<couchbaseclusters-spec-networking,networking>>:
    ...
    <<couchbaseclusters-spec-networking-adminconsoleservicetype,adminConsoleServiceType>>: ""
    ...
    <<couchbaseclusters-spec-networking-cloudnativegateway,cloudNativeGateway>>:
      ...
      <<couchbaseclusters-spec-networking-cloudnativegateway-resources,resources>>: {}
      <<couchbaseclusters-spec-networking-cloudnativegateway-servicetemplate,serviceTemplate>>: {}
      <<couchbaseclusters-spec-networking-cloudnativegateway-terminationgraceperiodseconds,terminationGracePeriodSeconds>>: 0
    ...
    <<couchbaseclusters-spec-networking-exposedfeatureservicetype,exposedFeatureServiceType>>: ""
  ...
  <<couchbaseclusters-spec-security,security>>:
    ...
    <<couchbaseclusters-spec-security-podsecuritycontext,podSecurityContext>>:
      <<couchbaseclusters-spec-security-podsecuritycontext-apparmorprofile,appArmorProfile>>:
        <<couchbaseclusters-spec-security-podsecuritycontext-apparmorprofile-localhostprofile,localhostProfile>>: ""
        <<couchbaseclusters-spec-security-podsecuritycontext-apparmorprofile-type,type>>: ""
    ...
    <<couchbaseclusters-spec-security-securitycontext,securityContext>>:
      ...
      <<couchbaseclusters-spec-security-securitycontext-apparmorprofile,appArmorProfile>>:
        <<couchbaseclusters-spec-security-securitycontext-apparmorprofile-localhostprofile,localhostProfile>>: ""
        <<couchbaseclusters-spec-security-securitycontext-apparmorprofile-type,type>>: ""
  ...
  - <<couchbaseclusters-spec-servers-autoscaleenabled,autoscaleEnabled>>: false
    ...
    <<couchbaseclusters-spec-servers-initcontainer,initContainer>>:
      <<couchbaseclusters-spec-servers-initcontainer-resources,resources>>: {}
  ...
  <<couchbaseclusters-spec-upgrade,upgrade>>:
    ...
    <<couchbaseclusters-spec-upgrade-rollbackmethod,rollbackMethod>>: SwapRebalance
...
=== couchbaseclusters.spec.buckets.selector.matchNames
...
their name.
If an object's name matches any of the provided patterns, it will be selected.
The patterns in this list are evaluated using OR logic.
Furthermore, the overall evaluation
...
=== couchbaseclusters.spec.cluster.data.kvThrottle
...
throughput (units per second) each bucket may use.
This protects against a "noisy
...
that the other buckets sharing the cluster suffer degraded performance.
It is most useful
...
=== couchbaseclusters.spec.cluster.data.kvThrottle.enabled
...
Enabled turns KV throughput throttling on for the whole cluster.
Per bucket reserved and
hard limit values, as well as nodeCapacity, can be configured while this is false, but they
only take effect once it is true.
Defaults to false.

=== couchbaseclusters.spec.cluster.data.kvThrottle.nodeCapacity
...
It is the shared pool that buckets draw from once they exceed their reservation.
It must be
greater than or equal to the sum of all bucket's throttleReserved values.
Leaving it unset
means unlimited (no cluster wide cap), which matches the behaviour when the feature is off.

=== couchbaseclusters.spec.cluster.data.magmaFlusherThreadPercentage
...
operations.
This is available in 7.6.10+, but not in 8.0.0.
This field is available in Couchbase Server 7.6.10 and later.
...
=== couchbaseclusters.spec.cluster.rebalance
...
=== couchbaseclusters.spec.cluster.rebalance.fileBasedMovesPerNode
...
streaming the data over DCP.
It applies only to the file based portion of a rebalance, DCP
moves are governed by spec.cluster.rebalance.movesPerNode.
Defaults to 4 on the server.
Leaving it unset while spec.cluster.rebalance is present resets it
to that default.
This field is available in Couchbase Server 8.1.0 and later.

=== couchbaseclusters.spec.cluster.rebalance.movesPerNode
...
and may degrade the performance of other systems including the Data Service.
Conversely a lower
...
=== couchbaseclusters.spec.networking.cloudNativeGateway.resources
...
=== couchbaseclusters.spec.security.podSecurityContext.appArmorProfile
...
=== couchbaseclusters.spec.security.podSecurityContext.appArmorProfile.localhostProfile
...
=== couchbaseclusters.spec.security.podSecurityContext.appArmorProfile.type
...
=== couchbaseclusters.spec.security.securityContext.appArmorProfile
...
=== couchbaseclusters.spec.security.securityContext.appArmorProfile.localhostProfile
...
=== couchbaseclusters.spec.security.securityContext.appArmorProfile.type
...
=== couchbaseclusters.spec.servers.initContainer
...
=== couchbaseclusters.spec.servers.initContainer.resources
...
Resources are the resource requirements for the init container.
If not
...
=== couchbaseclusters.spec.upgrade.rollbackMethod
...
*Enumerations*: `SwapRebalance, ConstrainedFailover, ConstrainedRebalanceOut`

==== Description

RollbackMethod selects how a rollback replaces nodes.
A rollback cannot be done
in place, so by default it swap rebalances, peaking at cluster size +
maxUpgradable pods and stalling where there is no room to schedule them.
The
...
*Enumerations*: `Available, Balanced, Scaling, ScalingUp, ScalingDown, Upgrading, Hibernating, Error, AutoscaleReady, Synchronized, WaitingBetweenMigrations, Migrating, Rebalancing, ExpandingVolume, BucketMigrating, BucketEvictionMigrating, Unreconcilable, WaitingBetweenUpgrades, MixedMode, ManualInterventionRequired, ServicesMismatch`
