...
<<couchbaseclusters-spec,spec>>:
  ...
  <<couchbaseclusters-spec-cluster,cluster>>:
    ...
    <<couchbaseclusters-spec-cluster-query,query>>:
      ...
      <<couchbaseclusters-spec-cluster-query-completedthreshold,completedThreshold>>: 1ms
  ...
  <<couchbaseclusters-spec-xdcr,xdcr>>:
    <<couchbaseclusters-spec-xdcr-globalsettings,globalSettings>>:
      ...
      <<couchbaseclusters-spec-xdcr-globalsettings-gogc,goGC>>: ""
...
requests catalog.
This field must either be a duration up to 2147483648ms, "0", or "-1".
...
In 2.9.0 this field is accepted by the CRD but
currently not implemented - usage will be rejected by the admission
controller.
Implementation (mounting) is planned for 2.9.1.
