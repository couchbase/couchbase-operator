...
<<couchbaseclusters-spec,spec>>:
  ...
  <<couchbaseclusters-spec-cluster,cluster>>:
    ...
    <<couchbaseclusters-spec-cluster-query,query>>:
      ...
      <<couchbaseclusters-spec-cluster-query-completedtrackingenabled,completedTrackingEnabled>>: True
      <<couchbaseclusters-spec-cluster-query-completedtrackingthreshold,completedTrackingThreshold>>: 7s
  ...
  <<couchbaseclusters-spec-networking,networking>>:
    ...
    <<couchbaseclusters-spec-networking-cloudnativegateway,cloudNativeGateway>>:
      <<couchbaseclusters-spec-networking-cloudnativegateway-dataapi,dataAPI>>:
        <<couchbaseclusters-spec-networking-cloudnativegateway-dataapi-enabled,enabled>>: false
        <<couchbaseclusters-spec-networking-cloudnativegateway-dataapi-proxyservices,proxyServices>>:
        - ""
...
per pod, to be altered.
This value must be between 4 and 64 threads for CB versions below 7.1.0 and,
...
per pod, to be altered.
This setting is especially relevant when
...
rules.
Disabling of address families is only supported in Couchbase
Server 7.0.2+.
...
=== couchbaseclusters.spec.networking.cloudNativeGateway.dataAPI
...
The DataAPI settings control whether the Cloud Native Gateway can be used to access the data api.
Adding or changing
...
=== couchbaseclusters.spec.networking.cloudNativeGateway.dataAPI.enabled
...
If set to true, the data api will be available on port 18008 of the cloud native gateway service.

=== couchbaseclusters.spec.networking.cloudNativeGateway.dataAPI.proxyServices
...
If this field is used, it must be one of "mgmt", "query", "search" or "analytics".
By default, none of these services will
...
*Enumerations*: `Available, Balanced, ManageConfig, Scaling, ScalingUp, ScalingDown, Upgrading, Hibernating, Error, AutoscaleReady, Synchronized, WaitingBetweenMigrations, Migrating, Rebalancing, ExpandingVolume`
