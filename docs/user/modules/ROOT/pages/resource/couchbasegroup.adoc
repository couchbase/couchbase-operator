...
<<couchbasegroups-status,status>>:
  <<couchbasegroups-status-conditions,conditions>>:
  - <<couchbasegroups-status-conditions-lasttransitiontime,lastTransitionTime>>: ""
    <<couchbasegroups-status-conditions-message,message>>: ""
    <<couchbasegroups-status-conditions-observedgeneration,observedGeneration>>: 0
    <<couchbasegroups-status-conditions-reason,reason>>: ""
    <<couchbasegroups-status-conditions-status,status>>: ""
    <<couchbasegroups-status-conditions-type,type>>: ""
----
...
When not specified for a role that can be scoped
...
When not specified,
the bucket field will be checked.
If both are empty and the role can be scoped to a specific bucket, the role
...
A Role
can only reference `CouchbaseBucket` kind.
This field defaults
...
for inclusion in this collection or collections.
If a resource reference doesn't
match a resource, then no error conditions are raised due to undefined
resource creation ordering and eventual consistency.
...
=== couchbasegroups.status.conditions
...
=== couchbasegroups.status.conditions.lastTransitionTime
...
=== couchbasegroups.status.conditions.message
...
=== couchbasegroups.status.conditions.observedGeneration
...
=== couchbasegroups.status.conditions.reason
...
=== couchbasegroups.status.conditions.status
...
=== couchbasegroups.status.conditions.type
