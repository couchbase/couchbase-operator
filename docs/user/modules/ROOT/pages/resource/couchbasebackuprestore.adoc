...
<<couchbasebackuprestores-spec,spec>>:
  ...
  <<couchbasebackuprestores-spec-continuousbackup,continuousBackup>>:
    <<couchbasebackuprestores-spec-continuousbackup-autoresolveconflicts,autoResolveConflicts>>: false
    <<couchbasebackuprestores-spec-continuousbackup-kms,kms>>:
      <<couchbasebackuprestores-spec-continuousbackup-kms-endpoint,endpoint>>: ""
      <<couchbasebackuprestores-spec-continuousbackup-kms-keyurl,keyUrl>>: ""
      <<couchbasebackuprestores-spec-continuousbackup-kms-region,region>>: ""
      <<couchbasebackuprestores-spec-continuousbackup-kms-secret,secret>>: ""
      <<couchbasebackuprestores-spec-continuousbackup-kms-useiam,useIAM>>: false
    <<couchbasebackuprestores-spec-continuousbackup-location,location>>: ""
    <<couchbasebackuprestores-spec-continuousbackup-restoreall,restoreAll>>: false
    <<couchbasebackuprestores-spec-continuousbackup-timestamp,timestamp>>: ""
  ...
  <<couchbasebackuprestores-spec-defaultrecoverymethod,defaultRecoveryMethod>>: none
  <<couchbasebackuprestores-spec-end,end>>:
    <<couchbasebackuprestores-spec-end-int,int>>: 0
    <<couchbasebackuprestores-spec-end-str,str>>: ""
  <<couchbasebackuprestores-spec-env,env>>:
  - <<couchbasebackuprestores-spec-env-name,name>>: ""
    <<couchbasebackuprestores-spec-env-value,value>>: ""
    <<couchbasebackuprestores-spec-env-valuefrom,valueFrom>>:
      <<couchbasebackuprestores-spec-env-valuefrom-configmapkeyref,configMapKeyRef>>:
        <<couchbasebackuprestores-spec-env-valuefrom-configmapkeyref-key,key>>: ""
        <<couchbasebackuprestores-spec-env-valuefrom-configmapkeyref-name,name>>: ""
        <<couchbasebackuprestores-spec-env-valuefrom-configmapkeyref-optional,optional>>: false
      <<couchbasebackuprestores-spec-env-valuefrom-fieldref,fieldRef>>:
        <<couchbasebackuprestores-spec-env-valuefrom-fieldref-apiversion,apiVersion>>: ""
        <<couchbasebackuprestores-spec-env-valuefrom-fieldref-fieldpath,fieldPath>>: ""
      <<couchbasebackuprestores-spec-env-valuefrom-resourcefieldref,resourceFieldRef>>:
        <<couchbasebackuprestores-spec-env-valuefrom-resourcefieldref-containername,containerName>>: ""
        <<couchbasebackuprestores-spec-env-valuefrom-resourcefieldref-divisor,divisor>>: ""
        <<couchbasebackuprestores-spec-env-valuefrom-resourcefieldref-resource,resource>>: ""
      <<couchbasebackuprestores-spec-env-valuefrom-secretkeyref,secretKeyRef>>:
        <<couchbasebackuprestores-spec-env-valuefrom-secretkeyref-key,key>>: ""
        <<couchbasebackuprestores-spec-env-valuefrom-secretkeyref-name,name>>: ""
        <<couchbasebackuprestores-spec-env-valuefrom-secretkeyref-optional,optional>>: false
  ...
  <<couchbasebackuprestores-spec-overwriteusers,overwriteUsers>>: false
  <<couchbasebackuprestores-spec-preserverestorerecord,preserveRestoreRecord>>: false
  ...
  <<couchbasebackuprestores-spec-services,services>>:
    ...
    <<couchbasebackuprestores-spec-services-users,users>>: false
...
<<couchbasebackuprestores-status,status>>:
  ...
  <<couchbasebackuprestores-status-completed,completed>>: false
  <<couchbasebackuprestores-status-conditions,conditions>>:
  - <<couchbasebackuprestores-status-conditions-lasttransitiontime,lastTransitionTime>>: ""
    <<couchbasebackuprestores-status-conditions-message,message>>: ""
    <<couchbasebackuprestores-status-conditions-observedgeneration,observedGeneration>>: 0
    <<couchbasebackuprestores-status-conditions-reason,reason>>: ""
    <<couchbasebackuprestores-status-conditions-status,status>>: ""
    <<couchbasebackuprestores-status-conditions-type,type>>: ""
...
=== couchbasebackuprestores.spec.continuousBackup
...
accepts a smaller set of options.
Setting this alongside spec.start, spec.end,
...
This field is available in Couchbase Server 8.1.0 and later.

=== couchbasebackuprestores.spec.continuousBackup.autoResolveConflicts
...
conflicts.
This is needed when a collection being recovered was dropped and recreated
before the restore, as it is given a new identifier and would otherwise not be matched
with the one in the backup.
Only use this when every scope and collection being recovered
serves the same purpose as the one it shares a name with.

=== couchbasebackuprestores.spec.continuousBackup.kms
...
were written with spec.continuousBackup.kmsKeyUrl set on the bucket.
Recovering encrypted
backups without it is not possible.

=== couchbasebackuprestores.spec.continuousBackup.kms.endpoint
...
=== couchbasebackuprestores.spec.continuousBackup.kms.keyUrl
...
*Pattern (Regular Expression)*: `^(awskms|gcpkms|azurekeyvault)://.+$`

==== Description

KeyURL identifies the key, and must be prefixed with "awskms://", "gcpkms://" or
"azurekeyvault://".
The prefix is what selects which key management system to talk to.
...
=== couchbasebackuprestores.spec.continuousBackup.kms.region
...
=== couchbasebackuprestores.spec.continuousBackup.kms.secret
...
It is mounted into the restore pod and its path handed to
the tool, rather than being passed on the command line, so that the credentials are not
readable from the pod specification.
The Secret must hold a single key, whose contents
...
=== couchbasebackuprestores.spec.continuousBackup.kms.useIAM
...
=== couchbasebackuprestores.spec.continuousBackup.location
...
spec.continuousBackup.location of the bucket that wrote them.
This is either a path on a
volume accessible to the restore, or an object storage URI prefixed with "s3://", "az://"
or "gs://".

=== couchbasebackuprestores.spec.continuousBackup.restoreAll
...
Exactly one of spec.continuousBackup.timestamp or
spec.continuousBackup.restoreAll must be set.

=== couchbasebackuprestores.spec.continuousBackup.timestamp
...
Timestamp is the point in time to recover to.
Exactly one of
...
=== couchbasebackuprestores.spec.defaultRecoveryMethod
...
=== couchbasebackuprestores.spec.env
...
=== couchbasebackuprestores.spec.env.name
...
=== couchbasebackuprestores.spec.env.value
...
=== couchbasebackuprestores.spec.env.valueFrom
...
=== couchbasebackuprestores.spec.env.valueFrom.configMapKeyRef
...
=== couchbasebackuprestores.spec.env.valueFrom.configMapKeyRef.key
...
=== couchbasebackuprestores.spec.env.valueFrom.configMapKeyRef.name
...
allowed to be empty.
Instances of this type with an empty value here are
almost certainly wrong.
More info: https://kubernetes.io/docs/concepts/overview/working-with-objects/names/#names.

=== couchbasebackuprestores.spec.env.valueFrom.configMapKeyRef.optional
...
=== couchbasebackuprestores.spec.env.valueFrom.fieldRef
...
=== couchbasebackuprestores.spec.env.valueFrom.fieldRef.apiVersion
...
=== couchbasebackuprestores.spec.env.valueFrom.fieldRef.fieldPath
...
=== couchbasebackuprestores.spec.env.valueFrom.resourceFieldRef
...
=== couchbasebackuprestores.spec.env.valueFrom.resourceFieldRef.containerName
...
=== couchbasebackuprestores.spec.env.valueFrom.resourceFieldRef.divisor
...
=== couchbasebackuprestores.spec.env.valueFrom.resourceFieldRef.resource
...
=== couchbasebackuprestores.spec.env.valueFrom.secretKeyRef
...
=== couchbasebackuprestores.spec.env.valueFrom.secretKeyRef.key
...
=== couchbasebackuprestores.spec.env.valueFrom.secretKeyRef.name
...
=== couchbasebackuprestores.spec.env.valueFrom.secretKeyRef.optional
...
=== couchbasebackuprestores.spec.preserveRestoreRecord
...
=== couchbasebackuprestores.status.completed
...
=== couchbasebackuprestores.status.conditions
...
=== couchbasebackuprestores.status.conditions.lastTransitionTime
...
=== couchbasebackuprestores.status.conditions.message
...
=== couchbasebackuprestores.status.conditions.observedGeneration
...
=== couchbasebackuprestores.status.conditions.reason
...
=== couchbasebackuprestores.status.conditions.status
...
=== couchbasebackuprestores.status.conditions.type
