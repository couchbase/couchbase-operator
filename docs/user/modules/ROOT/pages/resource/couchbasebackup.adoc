...
<<couchbasebackups-spec,spec>>:
  ...
  <<couchbasebackups-spec-env,env>>:
  - <<couchbasebackups-spec-env-name,name>>: ""
    <<couchbasebackups-spec-env-value,value>>: ""
    <<couchbasebackups-spec-env-valuefrom,valueFrom>>:
      <<couchbasebackups-spec-env-valuefrom-configmapkeyref,configMapKeyRef>>:
        <<couchbasebackups-spec-env-valuefrom-configmapkeyref-key,key>>: ""
        <<couchbasebackups-spec-env-valuefrom-configmapkeyref-name,name>>: ""
        <<couchbasebackups-spec-env-valuefrom-configmapkeyref-optional,optional>>: false
      <<couchbasebackups-spec-env-valuefrom-fieldref,fieldRef>>:
        <<couchbasebackups-spec-env-valuefrom-fieldref-apiversion,apiVersion>>: ""
        <<couchbasebackups-spec-env-valuefrom-fieldref-fieldpath,fieldPath>>: ""
      <<couchbasebackups-spec-env-valuefrom-resourcefieldref,resourceFieldRef>>:
        <<couchbasebackups-spec-env-valuefrom-resourcefieldref-containername,containerName>>: ""
        <<couchbasebackups-spec-env-valuefrom-resourcefieldref-divisor,divisor>>: ""
        <<couchbasebackups-spec-env-valuefrom-resourcefieldref-resource,resource>>: ""
      <<couchbasebackups-spec-env-valuefrom-secretkeyref,secretKeyRef>>:
        <<couchbasebackups-spec-env-valuefrom-secretkeyref-key,key>>: ""
        <<couchbasebackups-spec-env-valuefrom-secretkeyref-name,name>>: ""
        <<couchbasebackups-spec-env-valuefrom-secretkeyref-optional,optional>>: false
  ...
  <<couchbasebackups-spec-snapshotbackup,snapshotBackup>>:
    <<couchbasebackups-spec-snapshotbackup-maxskew,maxSkew>>: 10s
    <<couchbasebackups-spec-snapshotbackup-schedule,schedule>>: ""
    <<couchbasebackups-spec-snapshotbackup-volumegroupsnapshotclassname,volumeGroupSnapshotClassName>>: ""
    <<couchbasebackups-spec-snapshotbackup-volumesnapshotclassname,volumeSnapshotClassName>>: ""
...
<<couchbasebackups-status,status>>:
  ...
  <<couchbasebackups-status-conditions,conditions>>:
  - <<couchbasebackups-status-conditions-lasttransitiontime,lastTransitionTime>>: ""
    <<couchbasebackups-status-conditions-message,message>>: ""
    <<couchbasebackups-status-conditions-observedgeneration,observedGeneration>>: 0
    <<couchbasebackups-status-conditions-reason,reason>>: ""
    <<couchbasebackups-status-conditions-status,status>>: ""
    <<couchbasebackups-status-conditions-type,type>>: ""
...
=== couchbasebackups.spec.snapshotBackup
...
approach, which captures each member's underlying persistent volumes directly.
This
is faster to take and restore for large datasets, at the cost of a weaker consistency
guarantee.
Scheduling and retention behave the same as an archive based backup.

=== couchbasebackups.spec.snapshotBackup.maxSkew
...
per node).
If the time between the earliest and latest snapshot exceeds this, the
...
=== couchbasebackups.spec.snapshotBackup.schedule
...
CouchbaseBackupSchedule.Schedule.
A snapshot capture has no full, incremental, or
merge distinction the way an archive based backup does, so this is its own field.

=== couchbasebackups.spec.snapshotBackup.volumeGroupSnapshotClassName
...
member's volumes as a single group snapshot, when the driver supports it.
This is
...
=== couchbasebackups.spec.snapshotBackup.volumeSnapshotClassName
...
This is the universally available path, used whenever the
...
=== couchbasebackups.status.conditions
...
=== couchbasebackups.status.conditions.lastTransitionTime
...
=== couchbasebackups.status.conditions.message
...
=== couchbasebackups.status.conditions.observedGeneration
...
=== couchbasebackups.status.conditions.reason
...
=== couchbasebackups.status.conditions.status
...
=== couchbasebackups.status.conditions.type
