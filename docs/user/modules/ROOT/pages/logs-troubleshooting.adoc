...
You can even use the deployment to show the logs using the auto-generated name. The underlying command will automatically select a pod. In this case we only allow one, so it will behave correctly:
[source,console]
----
