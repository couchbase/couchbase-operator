...
*Default*: 0
+
Log level to be collected by 'cao collect-logs'
...
*Default*: default
+
Specifies the customer name for log uploading. This value must be a string whose maximum length is 50 characters. Only the following characters can be used: [A-Za-z0-9_.-].
...
*Type*: string
+
Name of an existing Kubernetes Secret containing 'tls.crt' and 'tls.key'. When set, the metrics server listens over HTTPS on the same port instead of plain HTTP.
...
== cao create pod [flags]

This command is for debug and recovery purposes only.  It is intended
...
Verifies the integrity of an archive collected using cao collect-logs
...
*Type*: string
+
Path to the target .tar.gz archive file
